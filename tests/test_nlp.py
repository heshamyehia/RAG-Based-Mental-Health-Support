...
def test_classify_intent_llm_label(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "models",
        SimpleNamespace(
            generate_content=lambda **kwargs: SimpleNamespace(text="greeting")
        ),
    )
...
def test_classify_intent_client_error(monkeypatch):
    ...
    monkeypatch.setattr(
        ic._client, "models", SimpleNamespace(generate_content=raise_error)
    )
...
def test_get_direct_response_success(monkeypatch):
    monkeypatch.setattr(
        ...
        "models",
        SimpleNamespace(
            generate_content=lambda **kwargs: SimpleNamespace(text="Hello there")
        ),
    )
...
def test_get_direct_response_on_exception_returns_fallback(monkeypatch):
    ...
    monkeypatch.setattr(
        ic._client, "models", SimpleNamespace(generate_content=bad_call)
    )
